...
%This is the master script for Approach 1, where the acceleration due to
...
ld_m = load('sensorlog_everythang.mat'); 
...
%Wanted to convert timesteps to real seconds, since 10 samples/sec or 10 Hz
%However, it needs to match the number of data points of the values
%themselves. Thus, decided to stick with time steps. 

%t_psi_d = [1:length(psi_d)/10];
...
% accelerometer's data (when in motion), where accelerometers -x and -y have been 
% equated to have around equal variances.) Thus (var(a_x ==var(a_y)).
% It is a square matrix that has the same size as the G matrix's number of columns.
...
figure(17);
plot(x_hat(2,:), y_hat(2,:), X_GPS, Y_GPS)
xlabel('X'); ylabel('Y')
legend('y-Kalman vs.x-Kalman','Y-GPS vs. X-GPS', 'location', 'southwest' )

%to run low pass filter or to run with a more exact aspect ratio run the
%script titled 'ME295B_Validation_ii'. 